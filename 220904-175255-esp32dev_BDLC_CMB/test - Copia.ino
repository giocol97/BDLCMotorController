...
bool updateState(int pulses, float speed, int millis)
{
  ...
  case STATE_RICARICA:
  {
    if (speed == 0)
    {
      ...
      if (speed == 0 && (millis - timeoutRicarica) > 3000)
      {
  ...
  {
    if (speed == 0 && millis - timeoutFineRicarica > 3000)
    {
      currentSystemState = STATE_INACTIVE;
      sensor.electric_rotations = 0;
  ...
  case STATE_CONFIGURAZIONE:
  {
    ...
    if (speed == 0 && millis - timeFromConfigStart > 100) // TODO filtro su velocità?
    {
      ...
      if (countConfigurationLeft == 1 && countConfigurationRight == 1) // configurati entrambi i lati, fine configurazione
      {
        ...
        sensor.electric_rotations = 0;
        currentSystemState = STATE_INACTIVE;
        countConfigurationLeft = 0;
        countConfigurationRight = 0;
  ...
  case STATE_INACTIVE:
    ...
    else if (millis - inactivityTimeout > 5 * 1000) // se inattivo per più di 300s passa in light sleep con sveglia da interrupt
    {
      inactivityTimeout = 0;
      digitalWrite(ENABLE_PIN, LOW);
      esp_sleep_pd_config(ESP_PD_DOMAIN_VDDSDIO, ESP_PD_OPTION_ON);

      if (digitalRead(HALL_U) == HIGH)
      {
        gpio_wakeup_enable((gpio_num_t)HALL_U, GPIO_INTR_LOW_LEVEL);
      }
      else
      {
        gpio_wakeup_enable((gpio_num_t)HALL_U, GPIO_INTR_HIGH_LEVEL);
      }
  ...
  while (1)
  {
    ...
    else
    {
      ...
      case STATE_CONFIGURAZIONE:
      {
        ...
        //motor.target = 35;
        if (abs(currentSpeed) < 45)
        {
          motor.target = 0.7 * configurationDirection;
        }

        if (abs(currentSpeed) > 55)
        {
          motor.target = 0.4 * configurationDirection;
        }
      }
      break;
      case STATE_RICARICA:
